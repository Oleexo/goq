...
| `seqcore` unit tests | §7.1 asks for table-driven tests per operator against `internal/seqcore`. Those direct tests do not exist; the functions are covered *transitively* through the fluent surface. The per-package coverage figure of 15% is a measurement artifact of omitting `-coverpkg`: measured across the module, every `seqcore` function is 69-100% covered, nothing is at zero, and the module total is 90.9%. So this is a strategy divergence, not a coverage hole — but it forfeits the "semantics tested independent of the fluent surface" property §3.3 wanted. |
