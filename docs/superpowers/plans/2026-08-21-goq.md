...
- [ ] **Step 4: Add the ordered branch to `parMap`'s consumption loop**
...
```go
				if !opts.ordered {
					...
					for res := range out {
						...
						if len(pending) > opts.window {
							// Defensive: unreachable given a bounded `out` and
							// blocking workers. Failing loudly beats growing
