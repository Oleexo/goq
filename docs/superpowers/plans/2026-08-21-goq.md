...
// Zip must not leak the pull-iterator coroutine it uses internally. goleak in
// TestMain is what actually catches a regression here.
func TestZipTerminatesOnInfinite(t *testing.T) {
	t.Parallel()
	got := goq.FromSeq(seqcore.Infinite()).
...
func TestChunkStreams(t *testing.T) {
	...
	got := goq.FromSeq(seqcore.Infinite()).Chunk(2).Select(func(b []int) int { return b[0] }).Take(2).ToSlice()
	if d := cmp.Diff([]int{0, 2}, got); d != "" {
		t.Errorf("infinite source mismatch (-want +got):\n%s", d)
	}
}
...
func TestDistinctStreams(t *testing.T) {
	t.Parallel()
	got := goq.Distinct(goq.FromSeq(seqcore.Infinite())).Take(3).ToSlice()
...
func TestIntersectStreamsReceiver(t *testing.T) {
	t.Parallel()
	got := goq.Intersect(goq.FromSeq(seqcore.Infinite()), goq.From([]int{2, 4})).Take(1).ToSlice()
